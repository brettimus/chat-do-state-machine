...
export const uiChatMachine = setup({
  ...
  on: {
    "chat.state.update": [
      ...
      {
        target: ".Loading",
        guard: ({ event }) => event.state === "Routing",
        actions: "storeServerContext",
      },
      {
        target: ".Loading",
        guard: ({ event }) => event.state === "FollowingUp",
        actions: "storeServerContext",
      },
      {
        target: ".StreamingResponse",
        guard: ({ event }) => event.state === "ProcessingAiResponse",
        actions: "storeServerContext",
      },
      {
        target: ".Loading",
        guard: ({ event }) => event.state === "GeneratingSpec",
        actions: "storeServerContext",
      },
      {
        target: ".Loading",
        guard: ({ event }) => event.state === "SavingSpec",
        actions: "storeServerContext",
      },
      {
        target: ".Done",
